...
class UserProfileRepository(UserProfileRepositoryInterface):
    ...
    @log_database_operation_decorator(operation_type='query', entity_type='user_practice_term_selection')
    @trace_database_operation(operation_type='select', table='user_practice_term_selections')
    @track_database_operation(operation_type='select', table='user_practice_term_selections')
    async def get_practice_term_selections(self, user_id: UUID) -> list[UserPracticeTermSelectionModel]:
        result = await self.db.execute(
            select(UserPracticeTermSelectionDataModel)
            .where(UserPracticeTermSelectionDataModel.user_id == user_id)
            .order_by(UserPracticeTermSelectionDataModel.added_at.desc())
        )
        return [
            UserPracticeTermSelectionModel(
                id=r.id,
    ...
    @log_database_operation_decorator(operation_type='create', entity_type='user_practice_term_selection')
    @trace_database_operation(operation_type='insert', table='user_practice_term_selections')
    @track_database_operation(operation_type='insert', table='user_practice_term_selections')
    async def add_practice_term_selection(
        self, user_id: UUID, practice_term_id: UUID,
    ) -> UserPracticeTermSelectionModel:
        sel = UserPracticeTermSelectionDataModel(
            id=uuid.uuid4(),
            user_id=user_id,
        ...
        return UserPracticeTermSelectionModel(
            id=sel.id,
            user_id=sel.user_id,
    ...
    @log_database_operation_decorator(operation_type='delete', entity_type='user_practice_term_selection')
    @trace_database_operation(operation_type='delete', table='user_practice_term_selections')
    @track_database_operation(operation_type='delete', table='user_practice_term_selections')
    async def remove_practice_term_selection(self, user_id: UUID, practice_term_id: UUID) -> bool:
        result = await self.db.execute(
            delete(UserPracticeTermSelectionDataModel).where(
                UserPracticeTermSelectionDataModel.user_id == user_id,
                UserPracticeTermSelectionDataModel.practice_term_id == practice_term_id,
            )
        )
