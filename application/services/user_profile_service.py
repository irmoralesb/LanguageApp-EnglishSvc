...
class UserProfileService:
    ...
    async def get_practice_term_selections(self, user_id: UUID) -> list[UserPracticeTermSelectionModel]:
        ...
        if existing is None:
            raise UserProfileNotFoundError(user_id)
        return await self.profile_repo.get_practice_term_selections(user_id)
    ...
    async def add_practice_term_selection(
        ...
        if existing is None:
            raise UserProfileNotFoundError(user_id)
        return await self.profile_repo.add_practice_term_selection(user_id, practice_term_id)

    async def remove_practice_term_selection(self, user_id: UUID, practice_term_id: UUID) -> bool:
        existing = await self.profile_repo.get_by_user_id(user_id)
        if existing is None:
            raise UserProfileNotFoundError(user_id)
        return await self.profile_repo.remove_practice_term_selection(user_id, practice_term_id)
